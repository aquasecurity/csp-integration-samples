...
for i in "${aws_regions[@]}"
do
   count=`aws elasticache describe-cache-clusters --region=$i --output json   |jq '.[] | length'`
...
do
   ...
   echo "Region=$i Applicaiton ELBs= "$count
   elbv2_count=`expr $elbv2_count + $count`
done
...
echo "Total count of NAT gateways across all regions: "$natgw_count
echo "Total count of Redshift clusters across all regions: "$redshift_count
echo "Total count of ELBs across all regions: "$elbv_count
echo "Total count of Application ELBs across all regions: "$elbv2_count
echo "Total billable resources:"$((ec2_instance_count+rds_instance_count+elb_count+elasticache_count+redshift_count+natgw_count+$elbv2_count))
